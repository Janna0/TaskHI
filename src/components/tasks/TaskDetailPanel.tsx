...
function CommentsSection({ taskId, projectId, memberMap }: {
  ...
  async function submit() {
    ...
    const { data: commentData } = await supabase
      ...
      .single()
    if (commentData) {
      if (activeTaskId.current === submittedTaskId) setComments(prev => [...prev, commentData as Comment])
      ...
      if (toNotify.length > 0) {
        await supabase.from('notifications').insert(
          toNotify.map(uid => ({ user_id: uid, actor_id: user.id, type: 'mention', task_id: submittedTaskId, project_id: projectId, comment_id: commentData.id }))
        )
      }
    }
    setInput('')
  ...
  return (
    <div>
          ...
          {comments.map(c => {
            ...
            return (
              <div key={c.id} className="flex gap-3 group">
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-semibold shrink-0 mt-0.5" style={{ background: authorColor }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    ...
                    {isOwn && (
                      <button onClick={async () => { await supabase.from('task_comments').delete().eq('id', c.id); loadComments(taskId) }}
                        className="opacity-0 group-hover:opacity-100 ml-auto text-slate-300 hover:text-red-400 transition-all">
                        <X size={11} />
                      </button>
                    )}
                  </div>
                  <p className="text-sm text-slate-600 leading-relaxed mt-0.5 whitespace-pre-wrap break-words">{renderContent(c.content)}</p>
                </div>
              </div>
            )
          })}
      ...
      <div className="flex gap-3 items-start">
        <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-semibold shrink-0 mt-0.5" style={{ background: myColor }}>
          {getInitials(myName)}
        </div>
          ...
          {mentionMatches.length > 0 && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-white border border-slate-200 rounded-lg shadow-lg overflow-hidden z-20">
              {mentionMatches.map(([uid, m]) => (
                <button key={uid} onMouseDown={e => { e.preventDefault(); insertMention(m.name) }}
...
function SubtaskRow({ sub, memberMap, onUpdate }: { sub: Task; memberMap: Record<string, { name: string; color: string }>; onUpdate: () => void }) {
  const [showAssignee, setShowAssignee] = useState(false)
  const pickerRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    function onClickOutside(e: MouseEvent) {
      if (pickerRef.current && !pickerRef.current.contains(e.target as Node)) setShowAssignee(false)
    }
  ...
  async function toggleAssignee(uid: string) {
    ...
    await supabase.from('tasks').update({ assignee_ids: current.includes(uid) ? current.filter(id => id !== uid) : [...current, uid] }).eq('id', sub.id)
    onUpdate()
  }
  ...
  return (
    <div className="flex items-center gap-2.5 py-1.5 px-2 -mx-2 rounded-lg hover:bg-slate-50 group">
      <button onClick={toggleDone}
        className={cn('w-4 h-4 rounded-full border-2 shrink-0 transition-colors flex items-center justify-center',
      ...
      <span className={cn('flex-1 text-sm min-w-0 truncate', sub.status === 'done' ? 'line-through text-slate-400' : 'text-slate-700')}>{sub.title}</span>
      {sub.due_date ? (
        <input type="date" value={sub.due_date} onChange={e => setDueDate(e.target.value)}
          className={cn('text-xs bg-transparent outline-none cursor-pointer shrink-0', isOverdue(sub.due_date) && sub.status !== 'done' ? 'text-red-400' : 'text-slate-400')} />
      ) : (
        <div className="relative shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" title="Set due date">
          <Calendar size={13} className="text-slate-300" />
          <input type="date" className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" onChange={e => setDueDate(e.target.value)} />
        </div>
      )}
      <div className="relative shrink-0" ref={pickerRef}>
        <button onClick={() => setShowAssignee(v => !v)}
          className={cn('flex items-center transition-opacity', assignees.length === 0 && 'opacity-0 group-hover:opacity-100')} title="Assign">
          {assignees.length > 0 ? (
            <div className="flex -space-x-1">
              {assignees.slice(0, 2).map((a, i) => (
                <div key={i} className="w-5 h-5 rounded-full border border-white flex items-center justify-center text-white text-[9px] font-semibold" style={{ background: a.color }}>{getInitials(a.name)}</div>
              ))}
            </div>
            ...
            {memberEntries.map(([uid, m]) => {
              ...
              return (
                <button key={uid} onClick={() => toggleAssignee(uid)}
                  ...
                  <div className={cn('w-5 h-5 rounded-full flex items-center justify-center text-white text-[9px] font-semibold border-2', selected ? 'border-primary-500' : 'border-transparent')} style={{ background: m.color }}>{getInitials(m.name)}</div>
                  <span className="text-sm text-slate-700">{m.name}</span>
...
export function TaskDetailPanel({ task, sections, memberMap, canEdit = true, onClose, onUpdated, onDeleted }: Props) {
  ...
  useEffect(() => {
    ...
    setDueDate(task.due_date ?? ''); setSectionId(task.section_id ?? '')
    setNotes(task.notes ?? ''); setAssigneeIds(task.assignee_ids ?? [])
    setHowToDocs(task.how_to_attachments ?? [])
    setCompetency(task.competency ?? ''); setTimeRequired(task.time_required ?? ''); setPhase(task.phase ?? '')
    setSaved(false); setShowDocPicker(false); setDocSearch('')
    loadSubtasks()
  }, [task.id])
  ...
  async function handleSave() {
    ...
    const { error: updateError } = await supabase.from('tasks').update({
      title: trimmedTitle, status, priority,
      due_date: dueDate || null, section_id: sectionId || null,
      notes: notes || null, assignee_ids: assigneeIds,
    ...
    if (added.length > 0) {
      await supabase.from('notifications').insert(added.map(id => ({ user_id: id, actor_id: user?.id, type: 'task_assigned', task_id: task.id, project_id: task.project_id })))
      ...
      ;(profiles ?? []).forEach(p => {
        supabase.functions.invoke('notify-assignment', { body: { assignee_email: p.email, assignee_name: p.name ?? p.email?.split('@')[0] ?? 'there', task_title: trimmedTitle, project_name: projectName, assigned_by_name: assignedByName } }).catch(() => {})
      })
    }
    onUpdated(); setSaving(false); setSaved(true)
  ...
  async function addSubtask() {
    ...
    if (!t) return
    await supabase.from('tasks').insert({ project_id: task.project_id, section_id: task.section_id, parent_task_id: task.id, title: t, position: subtasks.length, depth: (task.depth ?? 0) + 1 })
    setSubtaskInput('')
    loadSubtasks(); onUpdated()
  }
    ...
    <div className="w-[640px] shrink-0 border-l border-slate-200 bg-white flex flex-col h-full">
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          {canEdit && <button onClick={handleDelete} disabled={deleting} className="p-1.5 rounded-md text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors" title="Delete task"><Trash2 size={14} /></button>}
          {!canEdit && <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">View only</span>}
        </div>
        <div className="flex items-center gap-2">
          {canEdit && (saved ? (
            <span className="text-xs text-emerald-600 font-medium flex items-center gap-1">
              <svg className="w-3 h-3" fill="none" viewBox="0 0 12 12"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>Saved
            </span>
          ) : (
            <button onClick={handleSave} disabled={saving} className="text-xs font-medium bg-primary-500 hover:bg-primary-600 disabled:opacity-50 text-white px-3 py-1.5 rounded-md transition-colors">
              {saving ? 'Saving…' : 'Save'}
            </button>
          ))}
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-400 transition-colors"><X size={16} /></button>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto">
        <div className="px-6 pt-5 pb-2">
          <input className="w-full text-xl font-semibold text-slate-800 bg-transparent outline-none placeholder-slate-300 leading-snug read-only:cursor-default"
            value={title} readOnly={!canEdit} onChange={e => canEdit && setTitle(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur() }} placeholder="Task title" />
        </div>
        ...
        <div className="px-6 py-2 border-b border-slate-100">
          <PropRow icon={<User size={14} />} label="Assignee">
            <div className="relative" ref={assigneePickerRef}>
              <button onClick={() => canEdit && setShowAssigneePicker(v => !v)}
                ...
                {assigneeIds.length === 0 ? <span className="text-slate-400">No assignee</span> : (
                  <div className="flex items-center gap-1.5">
                    <div className="flex -space-x-1">
                      {assigneeIds.slice(0, 3).map(id => memberMap[id] && (
                        <div key={id} className="w-5 h-5 rounded-full border border-white flex items-center justify-center text-white text-[9px] font-semibold" style={{ background: memberMap[id].color }}>{getInitials(memberMap[id].name)}</div>
                      ))}
                    </div>
              ...
              {showAssigneePicker && memberEntries.length > 0 && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-30 p-2 min-w-[180px]">
                  {memberEntries.map(([uid, m]) => {
                    ...
                    return (
                      <button key={uid} onClick={() => setAssigneeIds(prev => prev.includes(uid) ? prev.filter(id => id !== uid) : [...prev, uid])}
                        className="flex items-center gap-2.5 w-full px-2 py-1.5 rounded-lg hover:bg-slate-50 text-left">
                        <div className={cn('w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-semibold border-2', selected ? 'border-primary-500' : 'border-transparent')} style={{ background: m.color }}>{getInitials(m.name)}</div>
                        <span className="text-sm text-slate-700">{m.name}</span>
                        {selected && <span className="ml-auto text-primary-500 text-xs">✓</span>}
                      </button>
                    )
          ...
          <PropRow icon={<Calendar size={14} />} label="Due date">
            <div className="flex items-center gap-1.5">
              <input type="date" className={cn('text-sm bg-transparent outline-none cursor-pointer', dueDate ? (isOverdue(dueDate) && status !== 'done' ? 'text-red-500' : 'text-slate-700') : 'text-slate-400')}
                value={dueDate} onChange={e => setDueDate(e.target.value)} />
              {dueDate && <button onClick={() => setDueDate('')} className="text-slate-300 hover:text-slate-500 transition-colors leading-none" title="Clear">×</button>}
            </div>
          </PropRow>

          <PropRow icon={<Flag size={14} />} label="Priority">
            <select className="text-sm bg-transparent outline-none cursor-pointer text-slate-700 hover:bg-slate-100 rounded px-1 -ml-1" value={priority} onChange={e => setPriority(e.target.value)}>
          ...
          {sections.length > 0 && (
            <PropRow icon={<Layers size={14} />} label="Status">
              <select className="text-sm bg-transparent outline-none cursor-pointer text-slate-700 hover:bg-slate-100 rounded px-1 -ml-1" value={sectionId}
                onChange={e => {
                  const newId = e.target.value; setSectionId(newId)
          ...
          <PropRow icon={<BookOpen size={14} />} label="How To">
            <div className="relative" ref={docPickerRef}>
              <button onClick={() => { setShowDocPicker(v => !v); if (!showDocPicker) { loadAvailableDocs(); setDocSearch('') } }}
                ...
                {howToDocs.length === 0 ? <span className="text-slate-400">Attach document</span> : (
                  <div className="flex items-center gap-1 flex-wrap">
                    {howToDocs.map(name => (
                      <span key={name} className="flex items-center gap-1 bg-primary-50 text-primary-700 text-xs rounded-md px-1.5 py-0.5">
                        <FileText size={10} />
                        <button onClick={e => { e.stopPropagation(); openDoc(name) }} className="max-w-[120px] truncate hover:underline">{name}</button>
                        <button onClick={e => { e.stopPropagation(); setHowToDocs(prev => prev.filter(n => n !== name)) }} className="text-primary-400 hover:text-red-400 ml-0.5 leading-none">×</button>
                      </span>
                    ))}
              ...
              {showDocPicker && (
                <div className="absolute left-0 top-full mt-1 w-72 bg-white border border-slate-200 rounded-xl shadow-lg z-30 overflow-hidden">
                  <div className="px-3 pt-2.5 pb-2 border-b border-slate-100">
                    <input autoFocus type="text" placeholder="Search documents…" value={docSearch} onChange={e => setDocSearch(e.target.value)}
                      className="w-full text-sm outline-none bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 placeholder-slate-400" />
                  </div>
                  {(() => {
                    const filtered = availableDocs.filter(d => !howToDocs.includes(d.name)).filter(d => d.name.toLowerCase().includes(docSearch.toLowerCase()))
                    return filtered.length === 0 ? (
                      <p className="px-4 py-3 text-xs text-slate-400">{availableDocs.length === 0 ? 'No How To documents available' : docSearch ? 'No matching documents' : 'All documents already attached'}</p>
                    ) : (
                      <div className="py-1 max-h-48 overflow-y-auto">
                        {filtered.map(doc => (
                          <button key={doc.id} onClick={() => { setHowToDocs(prev => [...prev, doc.name]); setShowDocPicker(false); setDocSearch('') }}
          ...
          <PropRow icon={<Award size={14} />} label="Competency">
            <select className="text-sm bg-transparent outline-none cursor-pointer text-slate-700 hover:bg-slate-100 rounded px-1 -ml-1" value={competency} onChange={e => setCompetency(e.target.value)}>
              <option value="">None</option>
              <option value="L1">L1</option><option value="L2">L2</option>
              <option value="L3">L3</option><option value="L4">L4</option>
            </select>
          </PropRow>

          <PropRow icon={<Clock size={14} />} label="Time Required">
            {(() => {
              const TIME_OPTIONS = ['A few minutes','About 30 minutes','About 1 hour','Half a day (4-8 hours)','1 day','A few days','1 week']
              ...
              return (
                <div className="flex items-center gap-2 flex-wrap">
                  <select className="text-sm bg-transparent outline-none cursor-pointer text-slate-700 hover:bg-slate-100 rounded px-1 -ml-1"
                    ...
                    onChange={e => { if (e.target.value === 'Other') setTimeRequired('__custom__'); else setTimeRequired(e.target.value) }}>
                    <option value="">None</option>
                    {TIME_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
                    <option value="Other">Other</option>
                  </select>
                  {(isCustom || timeRequired === '__custom__') && (
                    <input autoFocus type="text" placeholder="Custom time…"
          ...
          <PropRow icon={<Tag size={14} />} label="Phase">
            <select className="text-sm bg-transparent outline-none cursor-pointer text-slate-700 hover:bg-slate-100 rounded px-1 -ml-1" value={phase} onChange={e => setPhase(e.target.value)}>
              <option value="">None</option>
        ...
        <div className="px-6 py-4 border-b border-slate-100">
          ...
          <textarea className="w-full text-sm text-slate-600 bg-transparent resize-none outline-none placeholder-slate-400 leading-relaxed min-h-[80px]"
            placeholder="What is this task about?" value={notes} onChange={e => setNotes(e.target.value)} rows={4} />
        </div>

        <div className="px-6 py-4 border-b border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-slate-700">Subtasks</h3>
            <button onClick={() => document.getElementById('subtask-input')?.focus()} className="p-1 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors"><Plus size={15} /></button>
          </div>
          <div className="space-y-0.5 mb-2">
            {subtasks.map(sub => <SubtaskRow key={sub.id} sub={sub} memberMap={memberMap} onUpdate={loadSubtasks} />)}
          </div>
          <div className="flex items-center gap-3 py-1 px-2 -mx-2 rounded-lg border border-dashed border-slate-200 hover:border-slate-300 transition-colors focus-within:border-primary-300 focus-within:bg-primary-50/20">
            ...
            <input id="subtask-input" className="flex-1 text-sm text-slate-700 bg-transparent outline-none placeholder-slate-400 py-1.5"
              placeholder="Type to add a subtask…" value={subtaskInput} onChange={e => setSubtaskInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addSubtask() } if (e.key === 'Escape') setSubtaskInput('') }} />
          </div>
        </div>
