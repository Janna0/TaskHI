...
export function MyTasksPage() {
  ...
  useEffect(() => {
    if (!user) return
    const key = `mytasks:${user.id}`
    const cached = getCached<MyTasksCache>(key)
    if (cached) {
      setAssignedTasks(cached.assignedTasks)
      setProjectTasks(cached.projectTasks)
      setProjectNames(cached.projectNames)
  ...
  async function load(showLoader = true) {
    ...
    setCached(`mytasks:${user!.id}`, { assignedTasks: assignedList, projectTasks: projectList, projectNames: nameMap, memberMap: map })
