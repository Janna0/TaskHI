...
function AreaChart({ data }: { data: { label: string; total: number; completed: number }[] }) {
  if (data.length < 2) {
  ...
  const w = 480
  const h = 185
  const padL = 30, padR = 8, padT = 28, padB = 52
  ...
  const yAt = (v: number) => padT + (1 - v / maxVal) * chartH

  const pts = data.map((d, i) => ({ x: xAt(i), yT: yAt(d.total), yC: yAt(d.completed), ...d }))
  const totalLine = pts.map(p => `${p.x},${p.yT}`).join(' ')
  const completedLine = pts.map(p => `${p.x},${p.yC}`).join(' ')
  const totalArea = `M${pts[0].x},${baseY} L${totalLine} L${pts[pts.length-1].x},${baseY}Z`
  const completedArea = `M${pts[0].x},${baseY} L${completedLine} L${pts[pts.length-1].x},${baseY}Z`

  const step = Math.max(1, Math.ceil(data.length / 7))
  const labeled = pts.filter((_, i) => i % step === 0 || i === pts.length - 1)

  const yTicks = [0, 0.5, 1].map(pct => ({ y: padT + (1 - pct) * chartH, v: Math.round(pct * maxVal) }))

  return (
    <div className="overflow-x-auto">
      <svg width={w} height={h} style={{ overflow: 'visible' }}>
        <text x={10} y={padT + chartH / 2} textAnchor="middle" fontSize={9} fill="#94a3b8"
        ...
        {yTicks.map((g, i) => (
          <g key={i}>
            <line x1={padL} x2={padL + chartW} y1={g.y} y2={g.y} stroke="#f1f5f9" strokeWidth={1} />
            {g.v > 0 && <text x={padL - 3} y={g.y + 3} textAnchor="end" fontSize={9} fill="#94a3b8">{g.v}</text>}
          </g>
        ))}

        {/* Total area (light) */}
        <path d={totalArea} fill="#818cf8" fillOpacity={0.12} />
        <polyline points={totalLine} fill="none" stroke="#818cf8" strokeWidth={1.5} strokeOpacity={0.5} strokeLinejoin="round" />

        {/* Completed area (dark) */}
        <path d={completedArea} fill="#818cf8" fillOpacity={0.65} />
        <polyline points={completedLine} fill="none" stroke="#6366f1" strokeWidth={2} strokeLinejoin="round" />

        {/* Baseline */}
        <line x1={padL} x2={padL + chartW} y1={baseY} y2={baseY} stroke="#e2e8f0" strokeWidth={1} />

        {labeled.map((p, i) => (
          <g key={i}>
            <text x={p.x} y={p.yT - 5} textAnchor="middle" fontSize={9} fill="#64748b">{p.total}</text>
            {p.completed > 0 && (
              <text x={p.x} y={p.yC - 4} textAnchor="middle" fontSize={9} fill="#818cf8">{p.completed}</text>
            )}
            <text x={p.x} y={baseY + 6} textAnchor="end" fontSize={9} fill="#94a3b8"
              transform={`rotate(-45, ${p.x}, ${baseY + 6})`}>
              {p.label}
            </text>
          </g>
        ))}
      ...
      <div className="flex items-center gap-4 mt-1 pl-6">
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          <span className="w-3 h-3 rounded-sm bg-indigo-200 inline-block" /> Total
        </div>
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          <span className="w-3 h-3 rounded-sm bg-indigo-500 inline-block" /> Completed
        </div>
      </div>
    </div>
  )
...
export function DashboardView({ sections, tasks: allTasks, memberMap }: Props) {
  ...
  const overTime = useMemo(() => {
    ...
    const start = new Date(sorted[0].created_at); start.setHours(0, 0, 0, 0)
    const end = new Date(); end.setHours(23, 59, 59, 999)
    const result: { label: string; total: number; completed: number }[] = []
    const cur = new Date(start)
    while (cur <= end) {
      const snap = new Date(cur); snap.setDate(snap.getDate() + 1)
      const mm = String(cur.getMonth() + 1).padStart(2, '0')
      const dd = String(cur.getDate()).padStart(2, '0')
      result.push({
        label: `${mm}/${dd}`,
        total: tasks.filter(t => new Date(t.created_at) < snap).length,
        completed: tasks.filter(t => t.status === 'done' && new Date(t.updated_at) < snap).length,
      })
