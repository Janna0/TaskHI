    ...
    <Link href={`/projects/${project.id}/list`} className="block bg-white border border-[#e2e8f0] rounded-xl p-4 hover:border-[#6366f1]/40 hover:shadow-md transition-all group">
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center gap-1">
          {project.is_favorite && <Star className="h-3.5 w-3.5 text-amber-400 fill-amber-400" />}
      ...
      <div className="mt-2">
        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
          project.status === 'active' ? 'bg-[#eef2ff] text-[#6366f1]' :
...
function ProjectListRow({ project }: { project: Project }) {
  ...
  return (
    <Link href={`/projects/${project.id}/list`} className="flex items-center gap-4 px-4 py-3 hover:bg-[#f8fafc] transition-colors group">
      ...
      <span className="text-xs capitalize px-2 py-0.5 rounded bg-[#f1f5f9] text-[#64748b] hidden sm:block">{project.status}</span>
      {project.deadline && (
        <span className={`text-xs hidden sm:block w-20 text-right ${isOverdue ? 'text-[#dc2626]' : 'text-[#94a3b8]'}`}>
          {formatDate(project.deadline)}
        </span>
      )}
      {project.is_favorite && <Star className="h-3.5 w-3.5 text-amber-400 fill-amber-400 flex-shrink-0" />}
    </Link>
  );
}
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#64748b] mt-0.5">{projects.length} project{projects.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-[#94a3b8]">
          <p className="text-sm mb-4">
            {filter === 'favorites' ? 'No favorite projects yet.' :
             filter === 'archived' ? 'No archived projects.' :
             'No projects yet.'}
          </p>
          {filter === 'all' && (
            <Button size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" />Create project</Button>
          )}
        </div>
      ) : view === 'grid' ? (
        ...
        <div className="bg-white border border-[#e2e8f0] rounded-xl overflow-hidden">
          <div className="flex items-center gap-4 px-4 py-2 border-b border-[#e2e8f0] bg-[#f8fafc]">
            ...
            <span className="w-20 text-xs font-medium text-[#94a3b8] uppercase tracking-wider hidden sm:block">Status</span>
