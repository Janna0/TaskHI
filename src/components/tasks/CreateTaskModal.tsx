...
export function CreateTaskModal({ open, onClose, onCreated, projectId, sections, defaultSectionId }: Props) {
  ...
  function handleSelectTemplate(t: TemplateData) {
    setTemplate(t); setTitle(t.title); setError(''); setTab('new')
  }

  function reset() {
    setTab('new'); setTitle(''); setStatus('todo'); setPriority('medium')
    setDueDate(''); setTemplate(null); setError('')
  }
  ...
  async function handleSubmit() {
    ...
    if (err) {
      ...
      setError(isColErr ? 'Template fields require DB columns. Add competency, time_required, phase (text, nullable) to the tasks table in Supabase, then try again.' : err.message)
      return
    }
    ...
    <Modal open={open} onClose={() => { reset(); onClose() }} title="New Task">
      ...
      {tab === 'template' ? (
        ...
        <div className="space-y-3">
          ...
          <Input placeholder="Task title" value={title} onChange={e => { setTitle(e.target.value); setError('') }} error={error} autoFocus />
