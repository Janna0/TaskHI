...
export function PredefinedTasksSection({ mode = 'panel', onSelect }: Props) {
  ...
  const [newForm, setNewForm] = useState<NewForm>({ title: '', description: '', time_required: '', competency: '', phase: PREDEFINED_PHASES[0] })
  ...
  async function addTask() {
    ...
    setSaving(false)
    setAddingNew(false)
    setNewForm({ title: '', description: '', time_required: '', competency: '', phase: PREDEFINED_PHASES[0] })
    loadTasks()
  }
    ...
    return (
      <div className="py-2 px-3 bg-slate-50 rounded-lg border border-slate-200 space-y-2 mt-1 mb-1">
        <input autoFocus value={editForm.title || ''}
          ...
          onKeyDown={e => { if (e.key === 'Enter') saveEdit(); if (e.key === 'Escape') { setEditingId(null); setEditForm({}) } }}
        />
        ...
        <div className="flex gap-1.5">
          ...
          <button onClick={() => { setEditingId(null); setEditForm({}) }}
            className="text-xs text-slate-500 px-2 py-1 rounded-md hover:bg-slate-100">Cancel</button>
        </div>
      </div>
    )
  }
  ...
  function renderTaskRow(task: PredefinedTask) {
    if (editingId === task.id) return <div key={task.id}>{renderEditRow()}</div>
    return (
      <div key={task.id}
        onClick={isSelector ? () => onSelect?.({ title: task.title, description: task.description, time_required: task.time_required, competency: task.competency, phase: task.phase }) : undefined}
        className={cn('flex items-start gap-2 py-1.5 px-2 -mx-2 rounded-lg group transition-colors',
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {(task.time_required || task.competency) && (
            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
              {task.competency && <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded font-medium">{task.competency}</span>}
              {task.time_required && <span className="text-[10px] text-slate-400">{task.time_required}</span>}
            </div>
          )}
        </div>
        {!isSelector && (
          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5">
            <button onClick={e => { e.stopPropagation(); startEdit(task) }} className="p-1 rounded hover:bg-slate-200 text-slate-400 hover:text-slate-600"><Pencil size={11} /></button>
            <button onClick={e => { e.stopPropagation(); deleteTask(task.id) }} className="p-1 rounded hover:bg-red-50 text-slate-400 hover:text-red-500"><Trash2 size={11} /></button>
          </div>
        )}
      </div>
    )
    ...
    <div>
      ...
      {!loading && (
        <div className={cn('space-y-1', isSelector && 'max-h-72 overflow-y-auto pr-1')}>
          {grouped.length === 0 && <p className="text-xs text-slate-400 text-center py-4">No tasks found</p>}
          {grouped.map(group => {
            const isExpanded = !!search || expandedPhases.has(group.phase)
            return (
              <div key={group.phase}>
                <button onClick={() => togglePhase(group.phase)} className="flex items-center gap-1.5 w-full text-left py-1">
                ...
                {isExpanded && <div className="ml-3 space-y-0 mt-0.5">{group.tasks.map(t => renderTaskRow(t))}</div>}
              </div>
            )
          })}
        </div>
        ...
        <>
          {addingNew ? (
            <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-2">
              ...
              <input autoFocus value={newForm.title}
                onChange={e => setNewForm(f => ({ ...f, title: e.target.value }))}
                placeholder="Task title *"
                onKeyDown={e => { if (e.key === 'Escape') { setAddingNew(false); setNewForm({ title: '', description: '', time_required: '', competency: '', phase: PREDEFINED_PHASES[0] }) } }}
                className="w-full text-sm border border-slate-200 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-primary-300" />
              <textarea value={newForm.description} onChange={e => setNewForm(f => ({ ...f, description: e.target.value }))}
                placeholder="Description (optional)" rows={2}
                className="w-full text-xs border border-slate-200 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-primary-300 resize-none" />
              <div className="flex gap-2 flex-wrap">
                <select className={selectCls} value={newForm.time_required} onChange={e => setNewForm(f => ({ ...f, time_required: e.target.value }))}>
                  <option value="">No time</option>
                  {TIME_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
                <select className={selectCls} value={newForm.competency} onChange={e => setNewForm(f => ({ ...f, competency: e.target.value }))}>
                  <option value="">No level</option>
                  <option value="L1">L1</option><option value="L2">L2</option>
                  <option value="L3">L3</option><option value="L4">L4</option>
                </select>
                <select className={selectCls} value={newForm.phase} onChange={e => setNewForm(f => ({ ...f, phase: e.target.value }))}>
                  <option value="">No phase</option>
                  {PREDEFINED_PHASES.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div className="flex gap-1.5">
                ...
                <button onClick={() => { setAddingNew(false); setNewForm({ title: '', description: '', time_required: '', competency: '', phase: PREDEFINED_PHASES[0] }) }}
                  className="text-xs text-slate-500 px-2 py-1 rounded-md hover:bg-slate-100">Cancel</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setAddingNew(true)} className="mt-2 flex items-center gap-1.5 text-xs text-slate-400 hover:text-primary-500 transition-colors">
              <Plus size={12} /> Add predefined task
            </button>
          )}
        </>
      )}
  ...
  return (
    <div className="px-6 py-4 border-b border-slate-100">
      <button onClick={() => setSectionOpen(v => !v)} className="flex items-center gap-2 w-full text-left">
